...
class WeatherService {
    ...
    func fetchWeatherAlerts(for location: CLLocation) async throws -> [String] {
            ...
            if let weatherAlerts = weather.weatherAlerts {
                return weatherAlerts.map { alert in
                    "\(alert.summary): \(alert.detailsURL?.absoluteString ?? "")"
                }
            } else {
                return []
            }
    ...
    /// Method to determine the temperature unit based on locale
    /// This method expects temperature values in Fahrenheit from mock data
    /// Method to determine the temperature unit
    /// This method expects temperature values in Fahrenheit from the WeatherData model
    func getTemperatureInPreferredUnit(temperatureInFahrenheit: Double) -> String {
        if Locale.current.measurementSystem == .metric {
            // Convert Fahrenheit to Celsius for metric users
            let celsius = (temperatureInFahrenheit - 32) * 5 / 9
            return String(format: "%.1f°C", celsius)
        } else {
            // Return as Fahrenheit
            return String(format: "%.1f°F", temperatureInFahrenheit)
        }
    }

    // Alternative implementation that is clearer about units
    func formatTemperature(_ temperature: Double, unit: TemperatureUnit) -> String {
        switch unit {
        case .celsius:
            if Locale.current.measurementSystem == .metric {
                return String(format: "%.1f°C", temperature)
            } else {
                // Convert C to F for US users
                let fahrenheit = temperature * 9 / 5 + 32
                return String(format: "%.1f°F", fahrenheit)
            }
        case .fahrenheit:
            if Locale.current.measurementSystem == .metric {
                // Convert F to C for metric users
